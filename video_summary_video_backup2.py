...
ANALYSIS_PROMPT_HE = (
    "אתה בינה מלאכותית להשוואת תמונות בהם יש שינוי מהותי.\n"
    "\n"
    "הגדרת שינוי מהותי:\n"
    "• הופעה, היעלמות או שינוי משמעותי במיקום של אובייקט בולט בזירה, גדול או קטן, "
    "ובלבד שהוא משפיע על התפיסה הכללית של הזירה.\n"
    "• ההשוואה מתבצעת בין שתי תמונות תוך וידוא שהאובייקט או השינוי מתקיימים אך ורק באחת מהן.\n"
    "\n"
    "תהליך בדיקה:\n"
    "1. הופעה – בדוק אם אובייקט בולט קיים בתמונה אחת ואינו קיים בשנייה.\n"
    "2. היעלמות – בדוק אם אובייקט בולט נעלם מאחת התמונות ואינו מופיע באחרת.\n"
    "3. שינוי מיקום – בדוק אם אובייקט בולט זז באופן משמעותי.\n"
    "4. אימות נוכחות – אם אובייקט זהה מופיע בשתי התמונות באותו מיקום או במיקום דומה → אין שינוי מהותי.\n"
    "5. שינויי תאורה/צבע/צל קטנים או תזוזות זעירות → אינם שינוי מהותי.\n"
    "\n"
    "תגובה:\n"
    "• אם קיים שינוי מהותי – תאר אותו בבירור וציין אם זו הופעה, היעלמות או שינוי מיקום.\n"
    "• אם לא קיים שינוי מהותי – השב במדויק: \"אין שינוי מהותי.\""
)
...
def run_ground_change_detection():
    ...
    with st.expander("Advanced parameters"):
        ...
        # ------------ DIFF-MASK BOXES UI -----------
        st.session_state["use_diffmask_boxes"] = st.checkbox(
            "Use Diff‑mask bounding boxes (skip Mask‑R CNN/BIT)",
            value=False
        )
        st.session_state["dm_thr"] = st.slider(
            "Diff‑mask pixel threshold (0‑255)",
            min_value=5,
            max_value=100,
            value=25,
            step=1
        )
        st.session_state["dm_min_area"] = st.number_input(
            "Diff‑mask min box area (px)",
            min_value=10,
            value=200
        )
        # -------------------------------------------
        ...
        st.session_state["use_bit"] = st.checkbox(
        "Use BIT (skip Mask-R CNN)", value=False)
        st.session_state["bit_mask_thr"] = st.slider(
            "BIT mask threshold", 0.1, 0.9, 0.50, 0.05)
        st.session_state["bit_min_area"] = st.number_input(
            "BIT min box area (px)", min_value=10, value=200)
    if st.button("הכן זוגות"):
        # Determine source videos:
        if before_file is None and after_file is None:
            # Try default files in the working directory
            default_before = "לפני - גובה עשרים מטר.mp4"
            default_after  = "אחרי - גובה עשרים מטר.mp4"
            if not (os.path.exists(default_before) and os.path.exists(default_after)):
                st.error("לא הועלו קבצים ולא נמצאו סרטוני ברירת‑מחדל בספרייה המקומית.")
                return
            path_before = default_before
            path_after  = default_after
        elif before_file and after_file:
            # Save uploaded videos to a temp dir
            ...
            st.error("יש להעלות גם סרטון 'לפני' וגם 'אחרי' או להשאיר שניהם ריקים כדי להשתמש בברירת המחדל.")
            return
        # clean old state
        ...
        # keep temp dir so crops stay valid during session if uploads used
        ...
        if st.button("Analyze selected pairs") and selected_ids:
            ...
            def _process_pair(pair, custom_prompt):
                ...
                def _one_region(arg):
                    ...
                    if ("אין שינויים" not in gpt_txt) \
                        and ("לא נמצאו שינויים" not in gpt_txt) \
                        and ("אין שינוי מהותי" not in gpt_txt):
...
def _extract_focused_regions(img_ref, img_aligned,
    ...
    # ---------- STEP-2: optional segmentation filter (Mask-R CNN/BIT/DIFFMASK) ----------
    if use_segmentation:
        use_dm  = bool(st.session_state.get("use_diffmask_boxes", False))
        use_bit = bool(st.session_state.get("use_bit", False))
        ...
        for b64_r, b64_a, desc, diff in candidates:
            ...
            # ----- choose detector -----
            if use_dm:
                boxes = _diffmask_new_objects(
                    ref_tile, aln_tile,
                    thr=int(st.session_state.get("dm_thr", 25)),
                    min_area=int(st.session_state.get("dm_min_area", 200))
                )
            elif use_bit:
                boxes = _bit_new_objects(
                    ref_tile, aln_tile,
                    mask_thr=float(st.session_state.get("bit_mask_thr", 0.5)),
                    min_area=int(st.session_state.get("bit_min_area", 200))
                )
            else:
...
# Global singleton for ChangeFormer (lazy‑loaded)
...
# ---------- DIFF‑MASK BOXES (no deep model) ----------
def _diffmask_new_objects(img_before, img_after,
                          thr: int = 25,
                          min_area: int = 200):
    """
    Return bounding‑boxes [x1,y1,x2,y2] where absolute grayscale difference
    between `img_before` and `img_after` exceeds `thr`.
    Works entirely with OpenCV – no ML weights required.
    """
    import cv2, numpy as np
    g1 = cv2.cvtColor(np.array(img_before), cv2.COLOR_RGB2GRAY)
    g2 = cv2.cvtColor(np.array(img_after),  cv2.COLOR_RGB2GRAY)
    absdiff = cv2.absdiff(g1, g2)
    _, mask = cv2.threshold(absdiff, thr, 255, cv2.THRESH_BINARY)
...
def _get_bit_model():
    ...
    Tries multiple known repo aliases because some forks disappear:
      1. chengfeng-learning/BIT_CD
      2. likyoo/BIT_CD
      3. yangxy99/BIT_CD
    Returns None and logs error if all fail.
    """
    global _BIT_MODEL
    ...
    repo_candidates = [
        ("chengfeng-learning/BIT_CD", "BIT_S_R18"),
        ("likyoo/BIT_CD",            "BIT_S_R18"),
        ("yangxy99/BIT_CD",          "BIT_S_R18")
    ]
    for repo, model_name in repo_candidates:
        try:
            _BIT_MODEL = torch.hub.load(
                repo_or_dir=repo,
                model=model_name,
                pretrained=True,
                trust_repo=True
            ...
            _BIT_MODEL.eval()
            logging.info(f"Loaded BIT from {repo}")
            return _BIT_MODEL
        except Exception as e:
            logging.warning(f"BIT load failed from {repo}: {e}")

    st.error(
        "⚠️ Unable to download BIT weights from any known repo. "
        "Disable **Use BIT** or provide a local path."
    )
    _BIT_MODEL = None
    return None


def _bit_new_objects(img_before, img_after,
                     mask_thr: float = 0.5,
                     min_area: int = 200):
    model = _get_bit_model()
    if model is None:
        # Model failed to load; return no detections so pipeline continues gracefully
        return []
    import cv2
    from torchvision.transforms.functional import to_tensor, normalize, resize

    def _prep(pil):
        pil = resize(pil, [256, 256])
        t = to_tensor(pil)
        return normalize(t,
    ...
    with torch.no_grad():
        logits = model(_prep(img_before).unsqueeze(0),
                       _prep(img_after ).unsqueeze(0))[0]
    mask = torch.sigmoid(logits)[0].cpu().numpy()
    mask = cv2.resize(mask, img_before.size, interpolation=cv2.INTER_LINEAR)

    bin_m = (mask >= mask_thr).astype("uint8")*255
    bin_m = cv2.morphologyEx(
        bin_m, cv2.MORPH_CLOSE,
...
# ---------- CHANGEFORMER (lazy-loaded) ----------  # NEW
def _get_changeformer_model():
    """
    Load pretrained ChangeFormer-ResNet18 once.
    If internet is unavailable (404/timeout), show an error and return None.
    """
    global _CHANGEFORMER_MODEL
    if _CHANGEFORMER_MODEL is None:
        try:
            _CHANGEFORMER_MODEL = torch.hub.load(
                "likyoo/ChangeFormer",
                "changeformer_resnet18",
                pretrained=True,
                trust_repo=True
            )
            _CHANGEFORMER_MODEL.eval()
        except Exception as e:
            logging.error(f"ChangeFormer download failed: {e}")
            st.error(
                "⚠️ Unable to download ChangeFormer weights (offline?). "
                "Disable the **Use ChangeFormer** option or place weights locally."
            )
            _CHANGEFORMER_MODEL = None
    return _CHANGEFORMER_MODEL


def _changeformer_new_objects(img_before, img_after,
                              mask_thr: float = 0.5,
                              min_area: int = 200):
    """
    Return bounding-boxes [x1,y1,x2,y2] where ChangeFormer predicts change.
    """
    model = _get_changeformer_model()
    if model is None:
        # Return no boxes so pipeline falls back without crashing
        return []
    import cv2
    from torchvision.transforms.functional import to_tensor, normalize, resize
